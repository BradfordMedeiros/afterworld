...
vec3 textureWithFalloff(){
  float dist = length(_postColor - FragPos);
  ...
  const float falloffDistance = 2;
  float percentage = (falloffDistance - dist) / falloffDistance;
  if (percentage < 0){
    return vec3(0, 0, 0);
  }
  return 0.4 * texture(_overlayTexture, vec2(TexCoord.x * 10 , TexCoord.y * 10 )).rgb * percentage * 2;
...
void main(){
  ...
  vec3 effectColor = textureWithFalloff();
  ...
  for (int i = 0; i < 3; i++){
    ...
    float percentage = max(0, (5 - elapsedTime) / 5.f);


    if (distance < 2 && i == 0){
      extraColor = extraColor + vec4(percentage * 0.5, 0 , 0, 0);
      break;
    }
    if (distance < 2 && i == 1){
      extraColor = extraColor + vec4(0, percentage * 0.5 , 0, 0);
      break;
    }
    if (distance < 2 && i == 2){
      extraColor = extraColor + vec4(0, 0 , percentage * 0.5, 0);
      break;
    }
  }
