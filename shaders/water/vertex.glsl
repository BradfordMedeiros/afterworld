...
float waveLeftandRight = 0.5;
float waveForwardTurb = 1;
float waveSpeed = 0.4;
vec3 calcWaveOffset(vec3 position){
	return position + vec3(waveForwardTurb * cos(position.x + (waveSpeed * time)), waveLeftandRight * cos(position.x + (waveSpeed * time)), waveHeight  * cos(position.x + (waveSpeed * time)));
}
