...
vec4 calcWaveOffsetColor(vec3 position){
  return vec4(0, 0.05 * cos(position.x ), 0 * cos(position.x + time), 0) + (emissionAmount.x * vec4(1, 0, 0, 0));
}


vec2 scrollWater(vec2 coords){
	float yOffset = cos(time * 0.1);
...
//    texColor = texture(normalTexture, adjustedTexCoord).rgba;
    ...
    if (enableLighting){
      FragColor = (tint * ( calcWaveOffsetColor(FragPos) + vec4(color.xyz * shadowDelta, color.w))) * 2.5;
    }else{
