...
void drawBloom(objid playerId, objid id, float distance, float bloomAmount){
  ...
  auto mainobjPos = gameapi -> getGameObjectPos(playerId, true, "[gamelogic] drawBloom from playerId");
  auto mainobjRot = gameapi -> getGameObjectRotation(playerId, true, "[gamelogic] drawBloom from playerId"); // tempchecked
...
void drawWaypoints(Waypoints& waypoints, glm::vec3 playerPos){
  for (auto &[id, waypoint] : waypoints.waypoints){
    if (waypoint.id.has_value()){
      ...
      if (objectExists){
        auto position = gameapi -> getGameObjectPos(waypoint.id.value(), true, "[gamelogic] drawWaypoints - get waypoint locations");
