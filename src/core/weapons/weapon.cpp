...
std::optional<objid> raycastActivateableItem(objid playerId, bool anyAttr){
  auto hitpoints = doRaycastClosest(playerId, glm::vec3(0.f, 0.f, -1.f), std::nullopt, std::nullopt);
  if (hitpoints.size() > 0){
...
AmmoInfo currentAmmoInfo(WeaponEntityState& weaponState, objid inventory){
  auto gunName = weaponState.weaponValues.gunCore.weaponParams -> name;
  auto currentAmmo = ammoForGun(inventory, gunName);
  auto totalAmmo = weaponState.weaponValues.gunCore.weaponParams -> totalAmmo;
  return AmmoInfo {
...
WeaponsUiUpdate onWeaponsFrameEntity(WeaponEntityState& weaponState, objid inventory, objid playerId, glm::vec2 lookVelocity, glm::vec3 playerVelocity, bool showFpsGun, bool showThirdPersonGun, std::function<objid(objid)> getWeaponParentId, ThirdPersonWeapon thirdPersonWeapon, FiringTransform& fireTransform, bool isActivePlayer, bool isPlayerAlive){
  ...
  if(weaponState.holdToggle == HOLD_TOGGLE_PICKUP || weaponState.holdToggle == HOLD_TOGGLE_RELEASE){
    if (weaponState.heldItem.has_value()){
      ...
      auto hitpoints = doRaycast(glm::vec3(0.f, 0.f, -1.f), mainobjPos, mainobjRotation, std::nullopt);
      if (hitpoints.size() > 0){
...
std::vector<WeaponsUiUpdate> onWeaponsFrame(Weapons& weapons, objid playerId, glm::vec2 lookVelocity, glm::vec3 playerVelocity, std::function<WeaponEntityData(objid)> getWeaponEntityData, std::function<objid(objid)> getWeaponParentId, ThirdPersonWeapon thirdPersonWeapon, bool disableShowGun, bool isPlayerAlive) {
  ...
  for (auto &[id, weaponEntityState] : weapons.idToWeapon){
    ...
    auto uiUpdate = onWeaponsFrameEntity(weaponEntityState, weaponEntityData.inventory, id, weaponEntityData.lookVelocity, weaponEntityData.velocity, showWeaponViewModel && activePlayer && !weaponEntityData.thirdPersonMode && !disableShowGun, weaponEntityData.thirdPersonMode, getWeaponParentId, thirdPersonWeapon, weaponEntityData.fireTransform, activePlayer, isPlayerAlive);
    if (activePlayer){
